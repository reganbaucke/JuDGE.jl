...
function check_specification_is_legal(sub_problems)
   ...
   if !same_sense_at_each_node(sub_problems)
      message *= "Every subproblem must be a minimisation problem"
...
function same_expansion_constraints_at_each_node(subproblems)
   ec_of_subproblems = collect(map(get_structure_of_expansion_constraints,values(subproblems)))
...
function is_less_than(::ConstraintRef{Model,MathOptInterface.ConstraintIndex{MathOptInterface.ScalarAffineFunction{Float64},MathOptInterface.LessThan{Float64}},ScalarShape})
   true
end
...
function get_constraints_coeffs_for_expansions(subproblem)
   ...
   return Dict( (i,j) => thinwrapper(subproblem.obj_dict[i],subproblem.obj_dict[j]) for i in expkeys, j in expconkeys)
end
...
function get_expansion_keys(model)
   filter(keys(model.obj_dict)) do key
      for exp in model.ext[:expansions]
