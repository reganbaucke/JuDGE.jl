...
abstract type Functor{T} end

# mutable struct Node{T}
#    value::T
# end

# function Base.:(==)(a::Node{T}, b::Node{T}) where T
#    a.value == b.value
# end

struct IdFunct{T} <: Functor{T}
   value::T
end

struct Leaf{T} <: AbstractTree{T}
   data::T
end

# struct MyTree{T,S} <: AbstractTree{T} where S <: Functor{AbstractTree{T}}
#    data::T
#    children::S
# end

struct Tree{T,S} <: AbstractTree{T}
   data::T
   children::Functor{<:AbstractTree{T}}
end
...
#    children::Functor{<:AbstractTree{<:T}}
# end
where T = bounds(statqualia(T), xs)



hello = Tree([Tree(Leaf()),Leaf()])
...
function start(n::Node{Tree})
   return false
end

next(n::Node{Tree},state)  = (n,true)
done(n::Node{Tree},state)  = state
